...
class Output:
    ...
    def __repr__(self):
        ret = self.name + ": "
        ret += ",".join([str(resolution) for resolution in self.resolutions])
        return ret
...
def find_highest_mode(outputs):
    for r in res:
        for output in outputs:
            if r not in output.resolutions:
                break
        else:
            return r
...
def main():
    ...
    if doUseResolution:
        ...
        if outputs[0].supports(resolutionToUse) and outputs[1].supports(
            ...
            raise ValueError(
                "your two screens do not support resolution", resolutionToUse
            )
    else:
